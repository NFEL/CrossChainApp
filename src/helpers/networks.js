...
export const getNativeByChain = (chain) => chainList[chain]?.currencySymbol || "NATIVE";

export const getChainById = (chain) => chainList[chain]?.chainId || chainList[parseInt(chain, 16)]?.chainId || null;

export const getExplorer = (chain) => chainList[chain]?.explorers[0].url;

export const getWrappedNative = (chain) =>
  chainList[chain]?.wrapped || null;
